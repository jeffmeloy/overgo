...
func TestPairedProjectionCoordinatorRejectsInvalidState(t *testing.T) {
	...
	if _, err := runner.DraftPairedProjectionGreedy(t.Context(), target, 0, nil, 1, 0); err == nil {
		t.Fatal("nil draft session was accepted")
	}
	if _, err := runner.DraftPairedProjectionGreedy(t.Context(), target, 0, &PairedProjectionSession{}, 0, 0); err == nil {
	...
	if _, err := runner.VerifyPairedProjectionGreedy(t.Context(), target, draft); err == nil {
