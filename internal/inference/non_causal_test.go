...
func TestNonCausalRunnerRejectsCacheEntryPoint(t *testing.T) {
	runner := &Runner{spec: model.Spec{AttentionSpec: model.AttentionSpec{NonCausalAttention: true}}}
...
func TestEncoderRunnersRejectVocabularyLogits(t *testing.T) {
	for _, architecture := range []string{"bert", "llama-embed"} {
		t.Run(architecture, func(t *testing.T) {
			runner := &Runner{spec: model.Spec{CommonSpec: model.CommonSpec{Architecture: architecture}}}
