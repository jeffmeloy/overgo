...
func (r *Runner) ExtractLayerInputs(
	...
	for layerIndex, info := range r.weights.Layers {
		...
		activation, _, err = r.runLayerCached(
			ctx, activation, info, layerIndex, positions, nil, nil, reference.Value{}, nil, nil,
		)
		if err != nil {
...
func (r *Runner) DecodeDFlashNoiseBlock(
	...
	for layerIndex, info := range r.weights.Layers {
		activation, _, err = r.runLayerCached(
			ctx, activation, info, layerIndex, positions, nil, &cache.Layers[layerIndex], reference.Value{}, nil, nil,
		)
		if err != nil {
