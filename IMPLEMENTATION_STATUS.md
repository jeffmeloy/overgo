...
- RepoDB now executes compiled runtime workflows through registered typed module
  ...
  materialized inputs/outputs, dependency lineage, and successful, failed, or
  cancelled run records.
...
| Item | Current evidence | Completion boundary |
| --- | --- | --- |
| Indexed cache-target plan | Compiled graphs own output ordinals, alias contracts, indexed target slots, and indexed retained values; page-capacity buffers append in place without tensor-keyed target maps; each request still rebuilds cache schemas | Compiled session graph also owns capacity shapes and cache schemas; decode fills slices without schema lookup |
...
| Generated CUDA bindings | Manifest-generated kernel IDs, function table, module lookup, and drift tests replaced handwritten fields/lookups; argument layouts, launch validation, version, and hashes remain separate | Manifest or generated schema also drives argument layouts, launch validation, ABI version, and asset hashes |
