...
func TestPeerLifecycleAuthority(t *testing.T) {
	...
	lease, err := fixture.authority.Resolve(context.Background(), fixture.enrollment.ID, peerObservedUnixNS+1)
...
func TestPeerIdentityBinding(t *testing.T) {
	...
	if _, err := fixture.authority.Heartbeat(context.Background(), runrecord.PeerHeartbeat{
	...
	if _, _, err := fixture.authority.PublishCapability(context.Background(), fixture.enrollment.ID, modelrecipe.RemotePeerCapability{
...
func TestPeerLeaseExpiry(t *testing.T) {
	...
	defer fixture.store.Close()
	if _, err := fixture.authority.Resolve(context.Background(), fixture.enrollment.ID, peerExpiresUnixNS); err == nil {
		t.Fatal("expired peer lease was admitted")
	}
	if _, err := fixture.authority.Resolve(context.Background(), fixture.enrollment.ID, peerObservedUnixNS-1); err == nil {
...
func TestPeerDrainRefusal(t *testing.T) {
	...
	if _, err := fixture.authority.Transition(
		context.Background(), fixture.enrollment.ID, runrecord.PeerDraining, peerObservedUnixNS+1,
	); err != nil {
		t.Fatal(err)
	}
	if _, err := fixture.authority.Resolve(context.Background(), fixture.enrollment.ID, peerObservedUnixNS+2); err == nil {
	...
	if _, err := fixture.authority.Transition(
		context.Background(), fixture.enrollment.ID, runrecord.PeerRetired, peerObservedUnixNS+2,
	); err != nil {
		t.Fatal(err)
	}
	if _, err := fixture.authority.Heartbeat(context.Background(), runrecord.PeerHeartbeat{
...
func newPeerLifecycleFixture(t *testing.T, name string) peerLifecycleFixture {
	...
	publication, _, err := authority.PublishCapability(context.Background(), enrollment.ID, modelrecipe.RemotePeerCapability{
	...
	if _, err := authority.Heartbeat(context.Background(), runrecord.PeerHeartbeat{
...
func enrollPeerFixture(
	...
	enrollment, state, err := authority.Enroll(context.Background(), runrecord.PeerEnrollment{
