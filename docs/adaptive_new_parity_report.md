...
| SenseNova image/edit | Adaptive exact Go replay: 206.010 s, 19.50 GB; Python 366.6 s, 47.58 GiB | Fingerprinted retained-session two-step core: boundary cosine >=0.99914, velocity >=0.99863, final state >=0.99832; fresh cold body 3.33-7.37 s, warm body 3.28-3.32 s, 0.708 GiB peak; terminal normalized planar state publishes exact 256px PNG `d439b8ce...` | Warm-body lead; tensor-to-image publication gated; compiled request route open | Bind tokenizer/prompt, retained generation, and publication to the routed-image recipe; edit route still needs its absent source PNG |
