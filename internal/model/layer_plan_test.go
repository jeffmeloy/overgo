...
func TestPlanLayerCompilesDenseStagePolicies(t *testing.T) {
	tests := []struct {
		architecture string
		...
		{"olmo2", qkNormWeighted, qkNormNone, qkNormNone, residualSequential},
		{"qwen3", qkNormNone, qkNormWeighted, qkNormNone, residualSequential},
		{"maincoder", qkNormNone, qkNormNone, qkNormWeighted, residualSequential},
		{"gpt-oss", qkNormNone, qkNormNone, qkNormNone, residualGPTOSS},
		{"falcon", qkNormNone, qkNormNone, qkNormNone, residualFalcon},
	}
	for _, test := range tests {
		plan := Spec{CommonSpec: CommonSpec{Architecture: test.architecture}}.PlanLayer(0, false)
		if plan.QKPreprocess.Projection != test.projection ||
			plan.QKPreprocess.Heads != test.heads ||
			plan.QKPreprocess.PostRotary != test.postRotary ||
			plan.ResidualStages.kind != test.residual {
			t.Errorf("%s dense stages = %+v/%+v", test.architecture, plan.QKPreprocess, plan.ResidualStages)
		}
	}
}

func TestPlanLayerCompilesExpertComposition(t *testing.T) {
	tests := []struct {
		architecture string
		shared       uint32
		kind         expertCompositionKind
	}{
		{"qwen2moe", 1, expertSharedGated},
		{"step35", 1, expertSharedLimited},
		{"cohere2moe", 1, expertSharedAverage},
		{"grovemoe", 0, expertGrouped},
		{"grok", 0, expertGrok},
		{"arctic", 0, expertArctic},
	}
	for _, test := range tests {
		spec := Spec{
			CommonSpec: CommonSpec{Architecture: test.architecture},
			MoESpec:    MoESpec{SharedExpertFF: test.shared},
		}
		if got := spec.PlanLayer(0, false).ExpertComposition.kind; got != test.kind {
			t.Errorf("%s expert composition = %v, want %v", test.architecture, got, test.kind)
		}
	}
	invalid := Spec{
		CommonSpec: CommonSpec{Architecture: "grovemoe"},
		MoESpec:    MoESpec{ExpertCount: 3, ExpertsPerGroup: 2},
	}
	if err := invalid.PlanLayer(0, false).ExpertComposition.Validate(invalid); err == nil {
		t.Fatal("invalid GroveMoE grouping succeeded")
	}
}

func TestPlanLayerCompilesDenseGraphAndWeightContracts(t *testing.T) {
	tests := []struct {
		architecture string
		graph        DenseGraphPolicy
	}{
		{"bert", DenseGraphBERT},
		{"modern-bert", DenseGraphModernBERT},
		{"gemma4", DenseGraphGemma4},
		{"rwkv6qwen2", DenseGraphRWKV6Qwen2},
		{"llama", DenseGraphStandard},
	}
	for _, test := range tests {
		if got := (Spec{CommonSpec: CommonSpec{Architecture: test.architecture}}).PlanLayer(0, false).DenseGraph; got != test.graph {
			t.Errorf("%s dense graph = %v, want %v", test.architecture, got, test.graph)
		}
	}
	gptOSS := Spec{CommonSpec: CommonSpec{Architecture: "gpt-oss"}}.PlanLayer(0, false).DenseWeights
	if !gptOSS.requireOpenAIBiases || !gptOSS.requireAttentionOutputBias || !gptOSS.supportsExperts {
		t.Fatalf("GPT-OSS weight contract = %+v", gptOSS)
	}
	granite := Spec{
		CommonSpec: CommonSpec{Architecture: "granite"},
		MoESpec:    MoESpec{ExpertCount: 8, SharedExpertFF: 1},
	}.PlanLayer(0, false).DenseWeights
	if !granite.allowUngatedExperts || !granite.requireShared {
		t.Fatalf("Granite MoE weight contract = %+v", granite)
	}
}
