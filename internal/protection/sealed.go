...
// SealedAuthority is the owner-authored sealing contract: the OS principal
...
// LoadSealedAuthority reads the sealing manifest. A missing manifest reports
...
func LoadSealedAuthority(root string) (SealedAuthority, bool, error) {
	path := filepath.Join(root, filepath.FromSlash(sealedManifestPath))
	if _, err := os.Stat(path); errors.Is(err, os.ErrNotExist) {
		return SealedAuthority{}, false, nil
	}
	var authority SealedAuthority
	if err := jsonfile.DecodeStrict(path, &authority); err != nil {
		return SealedAuthority{}, false, fmt.Errorf("sealed authority manifest: %w", err)
	}
	if err := validateSealedAuthority(authority); err != nil {
		return SealedAuthority{}, false, err
	}
...
func (a SealedAuthority) Verify(root string) error {
...
func validateSealedAuthority(authority SealedAuthority) error {
