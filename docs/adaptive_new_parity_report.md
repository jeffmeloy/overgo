...
| Un-0 image/video | Active deterministic Go route; synthetic bootstrap training was deleted | Exact fixture within `3.13e-7`; about 1.25 ms warm generation. Full generator VJPs remain finite-difference and adaptive-golden gated. The prior constant color-ramp objective and its public `TrainDrift` entry point were deleted; training now refuses until a recipe supplies real class/image authority | Inference parity; honest training refusal | Publish encoded real-artifact image and video outputs through recipes; add real class/image training authority before restoring a trainer |
