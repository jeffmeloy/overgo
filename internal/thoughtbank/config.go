...
func DeriveArchFromShapes(shapes map[string][]int64, swiGLU bool) (ArchConfig, error) {
	...
	qn, err := get("blocks.0._attn.q_norm.weight", 1)
	if err != nil {
		return c, err
	}
	c.DHead = int(qn[0])

	wuq, err := get("blocks.0._attn.W_uq.weight", 2)
	if err != nil {
		return c, err
	}
	...
	if wdq, err := get("blocks.0._attn.W_dq.weight", 2); err != nil {
		return c, err
	...
	mnorm, err := get("blocks.0.mhc_attn.norm.weight", 1)
	if err != nil {
		return c, err
	}
	...
	w3, err := get("blocks.0._moe.experts.0.w3.weight", 2)
	if err != nil {
		return c, err
	}
	c.DFF = int(w3[1])
	...
	fwA, err := get("blocks.0.fw_A.weight", 2)
	if err != nil {
		return c, err
	}
	c.MemDim = int(fwA[1])
	...
	if fwB, err := get("blocks.0.fw_B.weight", 2); err != nil {
		return c, err
	...
	if nw, err := get("blocks.0.norm_fw.weight", 1); err == nil && int(nw[0]) != c.DModel {
	...
	if wiq, ok := shapes["blocks.0._attn.W_iq.weight"]; ok && len(wiq) == 2 && wiq[0]%int64(c.DHead) == 0 {
