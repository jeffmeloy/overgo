...
| Fractale-350M ThoughtBank | Shipped PyTorch checkpoint and tokenizer; retained reference completion for `def fibonacci(n):` | PyTorch tensor facts, tokenizer, checkpoint loader, incremental decode, and a retained host session execute through one typed generation recipe. The real 1.5 GB artifact produces the exact eight-token completion, publishes recipe-bound evidence, activates, and resolves the active program. Two matched six-token runs reproduce full-prefix logits exactly and take 1.444-1.467 s incrementally versus 2.333-2.351 s for repeated full-prefix execution, a 1.60-1.62x decode lead. Incremental prompt ingestion remains slower: 1.415-1.435 s versus 0.330-0.334 s | Exact real-output and logit parity; active generic recipe; decode wall lead | Replace token-at-a-time prompt ingestion with a cache-building full prefill; measure process peak and compare with the adaptive route |
