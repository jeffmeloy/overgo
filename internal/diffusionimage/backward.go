...
func tpaBackward(grads Grads, prefix string, x []float32, w tpaWeights, dOut []float32, seq, nEmbd, nHead, headDim, qRank, kvRank int, ropeTheta float64) []float32 {
	...
	linearF64(Aq, x, w.WAq, nil, seq, nEmbd, nHead*qRank)
	linearF64(Ak, x, w.WAk, nil, seq, nEmbd, nHead*kvRank)
	linearF64(Av, x, w.WAv, nil, seq, nEmbd, nHead*kvRank)
	linearF64(Bq, x, w.WBq, nil, seq, nEmbd, qRank*headDim)
	linearF64(Bk, x, w.WBk, nil, seq, nEmbd, kvRank*headDim)
	linearF64(Bv, x, w.WBv, nil, seq, nEmbd, kvRank*headDim)
...
func (blk *resBlock) backward(m *Model, x, dOut []float32, batch, channels, h, width int, grads Grads) []float32 {
	...
	groupNormBackward(dH1, hostmath.GradientSlot(grads, prefix+".norm2.weight", channels), hostmath.GradientSlot(grads, prefix+".norm2.bias", channels), h1, blk.norm2Weight, dN2, batch, channels, h, width, cfg.Groups, cfg.NormEps)
