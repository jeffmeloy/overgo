...
const (
	// StrategyMediaType identifies agent strategy profiles.
	StrategyMediaType = "application/vnd.overgo.agent-strategy+json"
	// StrategySchema identifies the agent strategy schema.
	StrategySchema = "overgo/agent-strategy/v1"
)

// Strategy is the immutable content-addressed bundle an attempt runs under:
// worker, prompt, model recipe, catalog, policies, and loop budgets.
...
// NewStrategy derives a canonical strategy from a validated worker
// definition, an exact catalog, and the loop budgets.
...
// Lineage links the strategy to its worker, prompt, model recipe, catalog, and policies.
...
// StampAttempt binds the strategy identity onto an attempt record and
// re-identifies the record.
...
// StampTrajectory binds the strategy identity onto an interaction trace and
// re-identifies it as an agent trajectory.
