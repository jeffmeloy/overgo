...
Metadata, strict catalog, graph semantics, and a complete reference/CUDA block
differential pass. `nomic-bert-moe` remains a separate architecture boundary.
...
ModernBERT remains deferred because its optional symmetric sliding window needs
a bidirectional local-attention mask. The current window primitive implements
decoder-style left-context clipping and cannot preserve ModernBERT semantics.
