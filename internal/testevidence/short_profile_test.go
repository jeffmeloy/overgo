...
func TestShortProfilePackageEvidence(t *testing.T) {
	...
	excluded := packageEvent("run", "example", "TestIntegration", "") +
		packageEvent("output", "example", "TestIntegration", ShortIntegrationSkip) + packageEvent("skip", "example", "TestIntegration", "")
	...
	for _, tc := range []struct {
		...
		{"unclassified skip", start + passed + strings.ReplaceAll(excluded, ShortIntegrationSkip, "fixture missing") + terminal, true, false},
		...
		{"late classification", start + passed + packageEvent("run", "example", "TestIntegration", "") + packageEvent("skip", "example", "TestIntegration", "") + packageEvent("output", "example", "TestIntegration", ShortIntegrationSkip) + terminal, true, false},
