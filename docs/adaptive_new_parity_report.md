...
| --- | --- | --- | --- | --- |
| Dense causal text: Carbon, Qwen2.5, MiniCPM | Active CUDA inference, real artifacts, capability floors | Weight and golden identities are live-gated; exact-token parity for all three; MiniCPM 3.109 vs 3.400 ms/token. Carbon conversion preserves the declared DNA-extension facts instead of publishing those vocabulary rows as unused padding. A fresh real-device gate serves all four adaptive cases through the active recipe; prompt/generated counts and greedy decoded text match exactly. The converted artifact is pinned at `08d12f90...`. Matched cold processes on the 32-token DNA case measure 2.305 s process / 2.217 s lifecycle / 463.992 MiB host / 1,045.098 MiB owned device for Overgo versus 1.871 s / 1.689 s / 3,884.645 MiB / 3,798.583 MiB for adaptive commit `214950b3b`. One identity-keyed MRU decode session now retains the compiled graph across requests while rebinding request-owned caches. Five-process alternating-order evidence measures 69.174 ms warm generation versus adaptive's 68.807 ms, inside the 2% parity band | Output and warm-wall parity; Carbon memory lead with cold-lifecycle deficit; MiniCPM wall lead | Close Carbon cold verification/lifecycle wall while retaining its 88.1% host and 72.5% device reductions; refresh Qwen2.5 wall and remaining matched peaks |
