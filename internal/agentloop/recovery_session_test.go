...
func TestAgentSessionRecovery(t *testing.T) {
	...
	leaseData, _ := json.Marshal(plan.WorkLease{Version: 1, Task: "recovery/session", Worktree: "C:/repo/recovery", Branch: "codex/recovery", Role: "developer", TargetHead: "0123456789abcdef0123456789abcdef01234567", ConflictsWith: []string{}, Resources: plan.Resources{CPUThreads: 1, HostRAMGiB: 1}, ExpiresAt: time.Now().Add(time.Hour).UTC().Format(time.RFC3339Nano)})
	lease, err := plan.RecordWorkLease(ctx, store, leaseData)
	if err != nil {
		t.Fatal(err)
	}
