...
| --- | --- | --- | --- | --- |
| Dense causal text: Carbon, Qwen2.5, MiniCPM | Active CUDA inference, real artifacts, capability floors | Weight and golden identities are live-gated; exact-token parity for all three; MiniCPM 3.109 vs 3.400 ms/token. Carbon conversion now preserves the declared DNA-extension facts instead of publishing those vocabulary rows as unused padding. A fresh real-device gate serves all four adaptive cases through the active recipe; prompt/generated counts and greedy decoded text match exactly. The converted artifact is pinned at `08d12f90...` | Parity; MiniCPM wall lead | Refresh Carbon/Qwen2.5 wall and all three matched peaks |
