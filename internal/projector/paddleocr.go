...
func (r *PaddleOCRRunner) patchEmbedding(ctx context.Context, input PaddleOCRImage) ([]float32, error) {
	weight, err := r.load(ctx, "v.patch_embd.weight")
	if err != nil {
		return nil, err
	}
	var bias []float32
	if hasTensor(r.file, "v.patch_embd.bias") {
		loaded, loadErr := r.load(ctx, "v.patch_embd.bias")
		if loadErr != nil {
			return nil, loadErr
		}
		bias = loaded.Data
...
func (r *PaddleOCRRunner) addPositions(ctx context.Context, hidden []float32, gridH, gridW int, rowOrder, columnOrder []int) error {
	...
	parallelRows(gridH*gridW, func(start, end int) {
		for token := start; token < end; token++ {
			...
			row := hidden[token*r.spec.Hidden:]
...
func (r *PaddleOCRRunner) normalize(ctx context.Context, input []float32, rows int, prefix string, epsilon float32) ([]float32, error) {
...
func (r *PaddleOCRRunner) runLayer(ctx context.Context, hidden []float32, rows int, rowOrder, columnOrder []int, layer int) error {
	prefix := fmt.Sprintf("v.blk.%d.", layer)
	normalized, err := r.normalize(ctx, hidden, rows, prefix+"ln1", r.spec.LayerNormEpsilon)
	if err != nil {
		return err
	}
	...
	attentionSpec := Qwen3VLSpec{Hidden: r.spec.Hidden, Heads: r.spec.Heads}
	attention := qwen3VLAttention(qkv, rows, rows, attentionSpec, rowOrder, columnOrder)
	...
	normalized, err = r.normalize(ctx, hidden, rows, prefix+"ln2", r.spec.LayerNormEpsilon)
	if err != nil {
		return err
	}
...
func (r *PaddleOCRRunner) projectQKV(ctx context.Context, input []float32, rows int, prefix string, layer int) ([]float32, error) {
...
func (r *PaddleOCRRunner) optionalBias(ctx context.Context, name string) ([]float32, error) {
...
func (r *PaddleOCRRunner) merge(ctx context.Context, hidden []float32, gridH, gridW int) (reference.Value, error) {
	normalized, err := r.normalize(ctx, hidden, gridH*gridW, "mm.input_norm", 1e-5)
	if err != nil {
		return reference.Value{}, err
	}
	mergedH := (gridH + r.spec.MergeSize - 1) / r.spec.MergeSize
	mergedW := (gridW + r.spec.MergeSize - 1) / r.spec.MergeSize
	mergedRows := mergedH * mergedW
	mergedWidth := r.spec.Hidden * r.spec.MergeSize * r.spec.MergeSize
	merged := make([]float32, mergedRows*mergedWidth)
	for blockY := 0; blockY < mergedH; blockY++ {
		for blockX := 0; blockX < mergedW; blockX++ {
			destination := (blockY*mergedW + blockX) * mergedWidth
			for y := 0; y < r.spec.MergeSize; y++ {
				for x := 0; x < r.spec.MergeSize; x++ {
					sourceY, sourceX := blockY*r.spec.MergeSize+y, blockX*r.spec.MergeSize+x
					if sourceY < gridH && sourceX < gridW {
						source := (sourceY*gridW + sourceX) * r.spec.Hidden
						copy(merged[destination:destination+r.spec.Hidden], normalized[source:source+r.spec.Hidden])
					}
	...
	fc1Weight, fc1Bias, err := r.loadPair(ctx, "mm.1.weight", "mm.1.bias")
	if err != nil {
		return reference.Value{}, err
	}
	fc1 := linear(merged, fc1Weight.Data, fc1Bias.Data, mergedRows, mergedWidth, r.spec.ProjectorIntermediate)
	r.activate(fc1)
	fc2Weight, fc2Bias, err := r.loadPair(ctx, "mm.2.weight", "mm.2.bias")
	if err != nil {
		return reference.Value{}, err
	}
	output := linear(fc1, fc2Weight.Data, fc2Bias.Data, mergedRows, r.spec.ProjectorIntermediate, r.spec.OutputHidden)
	return reference.NewValue(tensor.MustShape(uint64(r.spec.OutputHidden), uint64(mergedRows)), output)
}

func (r *PaddleOCRRunner) activate(values []float32) {
	for index, value := range values {
		switch r.spec.Activation {
		case paddleOCRGELU:
			values[index] = geluTanh(value)
		case paddleOCRSiLU:
			values[index] = value / (1 + float32(math.Exp(float64(-value))))
		default:
			values[index] = value / (1 + float32(math.Exp(float64(-1.702*value))))
		}
	}
}

func (r *PaddleOCRRunner) load(ctx context.Context, name string) (reference.Value, error) {
...
func (r *PaddleOCRRunner) loadPair(ctx context.Context, first, second string) (reference.Value, reference.Value, error) {
