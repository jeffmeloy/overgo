...
func render(rows []modelRow) []byte {
	...
	output.WriteString("Models are the servable catalog ordered smallest first by recorded bytes. Each suite cell is the headline metric of the model's latest committed evaluation of that suite under one prompting protocol; a dash means no committed evaluation. Suites are the store's derived benchmark suites (`evaluate -list-derived-suites`). The raw-completion protocol scores the suite's prompt text unchanged and is the recorded anchor comparable with lm-eval. The chat-template protocol wraps the prompt in the model's declared template and scores the answer as the first token of the model turn; for a template that opens the model turn with a thought channel, that position is not where the model answers, so those rows measure the template's fit to likelihood scoring rather than the model's knowledge.\n\n")
