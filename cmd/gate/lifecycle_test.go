...
func TestPreparedLifecycleLocatorBlocksAndRecovers(t *testing.T) {
	...
	if got, err := recordUnbatchableFailure(repo, storePath); err != nil || got != g.preparation.ID {
	...
	if _, err := recordUnbatchableFailure(repo, storePath); err != nil {
		t.Fatal(err)
	}
	if err := requireNoPendingGateState(repo, storePath); err != nil {
...
func TestUncommittedLifecycleLocatorIsRemoved(t *testing.T) {
	...
	if got, err := recordUnbatchableFailure(repo, storePath); err != nil || got != preparation.ID {
...
func TestUncommittedLifecycleLocatorIsRemovedAfterPriorFinalization(t *testing.T) {
	...
	if got, err := recordUnbatchableFailure(repo, storePath); err != nil || got != uncommitted.ID {
...
func TestRecordFailureClosesUniqueStalePreparationBehindFinalizedAuthority(t *testing.T) {
	...
	if got, err := recordUnbatchableFailure(fixture.repo, fixture.storePath); err != nil || got != fixture.stale[0].ID {
...
func TestRecordFailureWithFinalizedAliasRefusesPostCensusUnaliasedPreparation(t *testing.T) {
	...
	if _, err := recordUnbatchableFailure(fixture.repo, fixture.storePath); err == nil ||
...
func TestRecordFailureBootstrapsLegacyAliasToNewestTerminalAuthority(t *testing.T) {
	...
	if got, err := recordUnbatchableFailure(fixture.repo, fixture.storePath); err != nil ||
...
func TestRecordFailureRetryRepairsHeartbeatAfterLegacyStoreCommit(t *testing.T) {
	...
	if got, err := recordUnbatchableFailure(fixture.repo, fixture.storePath); got != fixture.outstanding[0].ID ||
...
func TestRecordFailureRemovesUncommittedRunningLocatorWhenLegacyDebtIsZero(t *testing.T) {
	...
	if got, err := recordUnbatchableFailure(fixture.repo, fixture.storePath); err != nil || got != uncommitted.ID {
