...
func writeWorkLeaseFixture(t *testing.T, root string) {
	t.Helper()
	data, err := json.Marshal(plan.WorkLease{
		Version: 1,
		...
		Resources: plan.Resources{CPUThreads: 8, HostRAMGiB: 16, VRAMGiB: 12}, ExpiresAt: "2099-01-01T00:00:00Z",
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestWorkLeaseRecord(t *testing.T) {
	...
	var output bytes.Buffer
	if err := printLeaseReport(root, plan.Resources{CPUThreads: 16}, &output); err != nil {
		t.Fatal(err)
	}
