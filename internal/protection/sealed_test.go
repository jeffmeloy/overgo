...
func TestCandidatePrincipalCannotWriteSealedAuthority(t *testing.T) {
	root := filepath.Join("..", "..")
	authority, sealed, err := LoadSealedAuthority(root)
	if err != nil {
		t.Fatal(err)
	}
...
func TestSealedAuthorityManifestValidation(t *testing.T) {
	root := t.TempDir()
	if _, sealed, err := LoadSealedAuthority(root); err != nil || sealed {
	...
	write := func(value SealedAuthority) {
	...
	for name, mutate := range map[string]func(*SealedAuthority){
		"no principal":         func(a *SealedAuthority) { a.CandidatePrincipal = " " },
		"no sealed paths":      func(a *SealedAuthority) { a.SealedPaths = nil },
		"absolute sealed path": func(a *SealedAuthority) { a.SealedPaths = []string{"C:/evil"} },
		"escaping sealed path": func(a *SealedAuthority) { a.SealedPaths = []string{"../outside"} },
		"missing provenance":   func(a *SealedAuthority) { a.Provenance = nil },
		"unsealed provenance": func(a *SealedAuthority) {
			a.Provenance[0].Path = "docs/other.json"
		},
		"anonymous tool": func(a *SealedAuthority) { a.Provenance[0].Tool = "" },
		"no version":     func(a *SealedAuthority) { a.Provenance[0].Version = " " },
		"bad digest":     func(a *SealedAuthority) { a.Provenance[0].SHA256 = "zz" },
	} {
		broken := manifest
		...
		mutate(&broken)
		write(broken)
		if _, _, err := LoadSealedAuthority(root); err == nil {
	...
	authority, _, err = LoadSealedAuthority(root)
	if err != nil {
		t.Fatal(err)
	}
	if err := authority.Verify(root); err == nil {
