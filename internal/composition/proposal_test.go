...
func TestBridgeProposalBlocksWithoutVerifier(t *testing.T) {
	...
	proposal, err := NewBridgeProposal(target, candidates, verifier, blocker)
	if err != nil {
		t.Fatal(err)
	}
	...
	replay, err := NewBridgeProposal(target, candidates, verifier, blocker)
	if err != nil || replay.ID != proposal.ID {
		t.Fatalf("replay = (%v, %v), want identical identity %v", replay.ID, err, proposal.ID)
	}

	if _, err := NewBridgeProposal(target, candidates, "", blocker); err == nil {
		t.Fatal("proposal without a verifier accepted")
	}
	if _, err := NewBridgeProposal(target, candidates, "echo ok", blocker); err == nil {
		t.Fatal("non-go-test verifier accepted")
	}
	if _, err := NewBridgeProposal(target, candidates, "go test ./internal/composition -count=1", blocker); err == nil {
		t.Fatal("verifier without a -run acceptance target accepted")
	}
	if _, err := NewBridgeProposal(target, candidates, verifier, "  "); err == nil {
		t.Fatal("proposal without a blocker reason accepted")
	}
	if _, err := NewBridgeProposal(target, nil, verifier, blocker); err == nil {
	...
	if err != nil || len(batch.Contents) != 1 || len(batch.Lineage) != 2 {
