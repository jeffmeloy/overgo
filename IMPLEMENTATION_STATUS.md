...
- Canonical model-definition documents now bind a model manifest, exact profile, tensor inventory, architecture, and fully validated runtime `Spec`. GGUF ingestion produces relocation-stable definitions; RepoDB resolves all bound facts and recipe v2 can compile a model plan from the stored definition without architecture-registry lookup.
