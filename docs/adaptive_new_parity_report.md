...
| Needle seq2seq | Inference only; no adaptive Needle training owner | Fingerprinted real weights/JAX oracle; retained load+generate envelope <=250 vs 420 ms. The typed text recipe uses the shared SentencePiece tokenizer and emits grounded JSON for the model-card weather tool-call example. Candidate execution published a recipe-bound gate/run pair; activation and active replay passed at the experimental tier. Native BF16 matrix ownership preserves the prior parity metrics and grounded generation while three matched cold processes measure 66.758-67.363 MiB versus adaptive_new's 120.918-121.328 MiB. A real GSM8K record crosses the shared materializer, typed paired-text processor, deterministic stream, tokenizer, shifted decoder targets, and shared cross-entropy. Shared reverse traversal now owns both final norms, every self/cross-attention block across all eight decoder layers, and every self-attention block across all 12 encoder layers: input/Q/K norms, raw residual gates, and Q/K/V/output projections. Decoder cross-attention K/V paths accumulate one encoder-memory gradient before the encoder VJP. All 226 compiled parameter groups have finite nonzero real-record gradients. The encoder final-norm analytic gradient `0.104070` matches the direct `0.103963` finite difference. Three device-Muon updates lower training loss from 6.541863 to 6.083478 and a separate held-out record from 8.629535 to 8.451201; 807,359 decoder and 611,787 encoder BF16 Q/K/V words change across every attention block. Exact JAX numeric parity, 65,536-value incremental/full identity, and the grounded tool-call generation remain unchanged | Exact numeric parity; real text generation; active recipe; wall and process-memory lead; real-data complete attention-stack device training with analytic-gradient and held-out evidence | Bind tied source/target/output embeddings; evaluate a larger fixed GSM8K split; do not claim adaptive training parity because adaptive_new has no Needle trainer |
