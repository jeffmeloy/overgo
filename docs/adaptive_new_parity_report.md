...
| Needle seq2seq | Inference only; no adaptive Needle training owner | Fingerprinted real weights/JAX oracle; retained load+generate envelope <=250 vs 420 ms. The typed text recipe uses the shared SentencePiece tokenizer and emits grounded JSON for the model-card weather tool-call example. Candidate execution published a recipe-bound gate/run pair; activation and active replay passed at the experimental tier. Native BF16 matrix ownership preserves the prior parity metrics and grounded generation while three matched cold processes measure 66.758-67.363 MiB versus adaptive_new's 120.918-121.328 MiB. A real GSM8K record crosses the shared materializer, typed paired-text processor, deterministic stream, tokenizer, shifted decoder targets, and shared cross-entropy. Compiled training now owns the decoder final norm and the complete last self/cross-attention pair: input/Q/K norms, raw residual gates, and Q/K/V/output projections. The shared causal and full-span attention VJPs are finite-difference gated. Real-record projection and norm gradients agree with parameter finite differences, including inverse RoPE and query-score scaling. With both final blocks active, three device-Muon updates lower training loss from 6.541863 to 6.405072 and a separate held-out record from 8.629535 to 8.534937. Final-self Q/K/V updates change 17,627/262,144, 15,203/131,072, and 12,245/131,072 BF16 words. Exact JAX numeric parity, 65,536-value incremental/full identity, and the grounded tool-call generation remain unchanged | Exact numeric parity; real text generation; active recipe; wall and process-memory lead; real-data complete-final-layer device training with analytic-gradient and held-out evidence | Propagate through the earlier decoder layers and encoder; evaluate a larger fixed GSM8K split; do not claim adaptive training parity because adaptive_new has no Needle trainer |
...
- Indexed reverse traversal now reaches decoder layer 6 cross-attention on the
  ...
  direct `-0.023562` finite difference. Its gate and output projection now
  share the Muon plan: three steps lower train loss `6.541863 -> 6.404089` and
  held-out loss `8.629535 -> 8.534847`, changing 17,434/262,144 output words.
  Layer 6 Q/K/V and normalization parameters remain open.
