...
| Image generation | Typed conditioning, CUDA-resident denoising, and PNG artifact output | Krea has verified 2048-pixel execution. SenseNova has a CUDA-tested 256-pixel text-to-PNG recipe. Un-0 and SimpleDiffusion publish real-checkpoint PNG artifacts that exactly match retained adaptive_new references. Full-size and image-edit tests are not complete. |
