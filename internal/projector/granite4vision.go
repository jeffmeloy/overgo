...
func (r *Granite4VisionRunner) encodeTile(ctx context.Context, tile Granite4VisionTile) ([]reference.Value, error) {
	...
	hidden := hostmath.LinearF64New(tile.PixelValues, patchWeight.Data, patchBias.Data, rows, patchWidth, r.spec.Hidden)
...
func (r *Granite4VisionRunner) runVisionLayer(ctx context.Context, hidden []float32, rows, layer int) error {
	...
	for partIndex, part := range []string{"q", "k", "v"} {
		...
		projected := hostmath.LinearF64New(norm, weight.Data, bias.Data, rows, r.spec.Hidden, r.spec.Hidden)
	...
	projected := hostmath.LinearF64New(attention, outWeight.Data, outBias.Data, rows, r.spec.Hidden, r.spec.Hidden)
	for index := range hidden {
	...
	up := hostmath.LinearF64New(norm, upWeight.Data, upBias.Data, rows, r.spec.Hidden, r.spec.Intermediate)
	hostmath.GELUTanhInPlace(up)
	downWeight, downBias, err := r.loadPair(ctx, prefix+"ffn_down.weight", prefix+"ffn_down.bias")
	if err != nil {
		return err
	}
	down := hostmath.LinearF64New(up, downWeight.Data, downBias.Data, rows, r.spec.Intermediate, r.spec.Hidden)
	for index := range hidden {
...
func (r *Granite4VisionRunner) runQFormerBlock(ctx context.Context, hidden []float32, side, block int) ([]float32, error) {
	...
	up := hostmath.LinearF64New(cross, upWeight.Data, upBias.Data, windows*queryLength, r.spec.Hidden, r.spec.QFormerWidth)
	...
	ffn := hostmath.LinearF64New(up, downWeight.Data, downBias.Data, windows*queryLength, r.spec.QFormerWidth, r.spec.Hidden)
	for index := range ffn {
	...
	return hostmath.LinearF64New(unwinned, linearWeight.Data, linearBias.Data, newSide*newSide, r.spec.Hidden, r.spec.ProjectionDim), nil
}
...
func (r *Granite4VisionRunner) qformerAttention(
	...
	for index, name := range []string{"q", "k", "v"} {
		...
		parts[index] = hostmath.LinearF64New(input, weight.Data, bias.Data, rows, r.spec.Hidden, r.spec.Hidden)
	}
	...
	return hostmath.LinearF64New(attention, outWeight.Data, outBias.Data, windows*queryRows, r.spec.Hidden, r.spec.Hidden), nil
}
