...
func TestRSICampaignRatchetAndParallelStructure(t *testing.T) {
	...
	wantDependencies := map[string][]string{
		...
		"architecture-closure/single-entry-guards":             {"architecture-ratchets/go-only-guard", "interaction-efficiency/efficiency-gate", "model-recipe-lifecycle/recipe-activation", "evidence-routing/counterfactual-replay", "deterministic-rollout/promotion-binding", "live-safety/reentry"},
