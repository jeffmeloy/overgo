	...
	for t := 0; t < T; t++ {
		...
		for li := range m.layers {
			...
			for head := 0; head < h; head++ {
				hostmath.ApplyRotaryInterleaved(q[head*hd:(head+1)*hd], m.invFreq, pos)
				hostmath.ApplyRotaryInterleaved(k[head*hd:(head+1)*hd], m.invFreq, pos)
			}
			st.k[li] = append(st.k[li], k...)
			st.v[li] = append(st.v[li], v...)
			for i := range q {
				q[i] *= m.scoreScale
			}
			hostmath.CausalAttentionStep(attn, q, st.k[li], st.v[li], pos+1, h, h, hd)
