...
| Image generation | Typed conditioning, CUDA-resident denoising, and PNG artifact output | Krea has verified 2048-pixel execution. SenseNova has a CUDA-tested 256-pixel text-to-PNG recipe. Un-0 and SimpleDiffusion publish real-checkpoint PNG artifacts that exactly match retained adaptive_new references. SimpleDiffusion also beats adaptive_new in a matched cold-process host-forward gate. Full-size and image-edit tests are not complete. |
| Video generation | Typed video recipes; GIF publication for class-conditioned oscillator output; CUDA-resident Wan denoising and VAE decoding; LiveEdit schedule, checkpoint selection, reference trajectory, and shared text conditioning | Un-0 publishes six real-artifact frames as a 64x64 GIF byte-identical to adaptive_new. Wan has verified full-clip execution. LiveEdit source encoding, resident denoising, production activation, and performance comparison are not complete. |
