...
| LiveEdit video edit | Adaptive complete experimental route; best 109.5 s, 20.61 GB; Python 60.1 s | The shared source-codec boundary derives causal `[1,stride,...]` frame chunks, latent geometry, channel-major frame spans, and recipe-owned latent normalization. The real Wan VAE compiles into 17 ordered encoder operations with 86 owned tensors and 204.456 MiB of weights. Its resident source encoder reuses the decoder's causal convolution, RMS normalization, attention, cache, and activation owners; only asymmetric spatial and causal temporal downsampling were added. A five-crop real Wan-frame case maps 5x32x32 source to 2x4x4 latent in two chunks: 0.055 s CUDA versus 2.187 s host, 218.858 MiB device peak, and `3.278e-6` maximum host-oracle error. The neutral diffusion graph now accepts retained rotated self-attention K/value history. A real two-block LiveEdit checkpoint run projects captured text context once, executes two sequential chunks from a real encoded source plus a retained Wan latent, keeps a two-token history tail on-device, uploads 0.174 GiB of BF16 weights, and peaks at 0.212 GiB; retained history changes the second-chunk output by max absolute `0.541469` versus a cold graph. A typed bounded runtime composes source encode, all 30 checkpoint blocks, BF16 sampler arithmetic, history commit/reset, causal decode, and encoded publication. The production recipe distinguishes conditioning from source video, uses the shared multi-input resident cache, and publishes borrowed decoded frames directly into a deterministic GIF. Two executions of the real 5x16x32 case reuse one session and emit the same 4,937-byte, five-frame GIF (`5828c516…`); 2,048 pixels change across adjacent frames. | Full checkpoint execution, typed activation, encoded publication, and exact bounded resident replay proven | Compare full native source/edit output and beat Python without temporal degradation |
...
encoded clip publication, and repeated resident-request evidence at bounded
geometry. Its remaining gap is a matched full native edit and leadership gate.
