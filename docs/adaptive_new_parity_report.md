...
| --- | --- | --- | --- | --- |
| Dense causal text: Carbon, Qwen2.5, MiniCPM | Active CUDA inference, real artifacts, capability floors | Weight and golden identities are live-gated; exact-token parity for all three; MiniCPM 3.109 vs 3.400 ms/token. Carbon conversion preserves the declared DNA-extension facts instead of publishing those vocabulary rows as unused padding. A fresh real-device gate serves all four adaptive cases through the active recipe; prompt/generated counts and greedy decoded text match exactly. The converted artifact is pinned at `08d12f90...`. Carbon retains one identity-keyed decode session across requests while rebinding request-owned caches. One-shot prefill retains output buffers without entering the replay cache, reducing 12-generation graph instantiations from 25 to 4 and graph launches from 384 to 372. The shared SHA-256 owner uses a bounded 1 MiB transfer buffer while still hashing every artifact byte. Five-process alternating-order evidence with 11 warm generations per process measures 2.216 s cold lifecycle / 463.512 MiB host / 1,045.098 MiB owned device and 65.853 ms warm generation for Overgo versus 1.693 s / 3,884.516 MiB / 3,798.583 MiB and 68.796 ms for adaptive commit `214950b3b`. Overgo's median cold phases are 1.725 s verified resolution, 332.1 ms open/upload, and 161.9 ms first generation | Exact output; 4.3% warm-wall lead; 88.1% host and 72.5% device-memory reductions; cold deficit bounded below 1.5x; MiniCPM wall lead | Reduce Carbon verified resolution without weakening full-content identity; refresh Qwen2.5 wall and remaining matched peaks |
| Fractale-350M ThoughtBank | Shipped PyTorch checkpoint and tokenizer; retained reference completion for `def fibonacci(n):` | PyTorch tensor facts, tokenizer, checkpoint loader, incremental decode, and a retained host session execute through one typed generation recipe. The real 1.5 GB artifact produces the exact eight-token completion, publishes recipe-bound evidence, activates, and resolves the active program. Full-forward prompt processing now builds every attention cache in 0.389-0.462 s versus 1.715-1.901 s for the removed token-at-a-time path, a 3.71-4.89x lead; cached continuation remains bit-exact. Two matched six-token runs reproduce full-prefix logits exactly and take 1.411-1.532 s incrementally versus 2.348-2.625 s for repeated full-prefix execution, a 1.66-1.71x decode lead. Three isolated 20-token parity processes measure a 6.239 s median and 1,793.934 MiB median peak versus 8.354 s and 1,810.918 MiB for adaptive commit `b8fef3cc2` | Exact real-output, prefill, and decode parity; active generic recipe; 25.3% matched process-wall lead and 16.984 MiB peak lead | Complete block/model training and checkpoint/resume through shared owners |
