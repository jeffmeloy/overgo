...
func TestDeferredLaneObligations(t *testing.T) {
	...
	retainedObligation, found, err := runrecord.CurrentGateLaneObligation(t.Context(), store)
	if err != nil || !found || retainedObligation.ID != running.ID {
		t.Fatalf("resuming changed the durable obligation: %+v %v", retainedObligation, err)
	}
