...
| Needle seq2seq | Partial adaptive forward/backward | Fingerprinted real weights/JAX oracle; retained load+generate envelope <=250 vs 420 ms. The compiled recipe now accepts text, uses the shared SentencePiece tokenizer, and emits grounded JSON for the model-card weather tool-call example | Exact numeric parity; real text generation; wall lead | Activate with refreshed evidence; matched peak measurement; approved fine-tuning objective and dataset |
