...
| LiveEdit video edit | Adaptive complete experimental route; best 109.5 s, 20.61 GB; Python 60.1 s | The shared source-codec boundary derives causal `[1,stride,...]` frame chunks, latent geometry, channel-major frame spans, and recipe-owned latent normalization. The real Wan VAE compiles into 17 ordered encoder operations with 86 owned tensors and 204.456 MiB of weights. Its resident source encoder reuses the decoder's causal convolution, RMS normalization, attention, cache, and activation owners; only asymmetric spatial and causal temporal downsampling were added. A five-crop real Wan-frame case maps 5x32x32 source to 2x4x4 latent in two chunks: 0.055 s CUDA versus 2.187 s host, 218.858 MiB device peak, and `3.278e-6` maximum host-oracle error | Real source graph, channel-major execution, and normalized latent seam proven | Retain source latent through denoise; verify the missing native edit-source oracle when supplied; beat Python without temporal degradation |
