...
func TestContractPackagesHaveNoPackageLocalExports(t *testing.T) {
	...
	for _, declaration := range declarations {
		if !declaration.Exported || declaration.Boundary != "" ||
			declaration.ProductionReferences != 0 || declaration.TestReferences != 0 || declaration.ExternalReferences != 0 {
			continue
		}
