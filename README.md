...
| Image generation | Typed conditioning, CUDA-resident denoising, and PNG artifact output | Krea has verified 2048-pixel execution. SenseNova has a CUDA-tested 256-pixel text-to-PNG recipe. Un-0 publishes a real-artifact PNG that is pixel-exact against adaptive_new. Full-size and image-edit tests are not complete. |
