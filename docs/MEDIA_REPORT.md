...
Outputs from recorded successful generation runs, exported by `go run ./cmd/compatibility -export-samples` and named by content SHA-256. A successful run does not by itself establish visual quality.
