...
| Client style ratchet at zero (`webuiReviewCeiling` inline styles, nested ternaries and timer literals all 0; utility and component classes in `style.css`; the `hidden` attribute enforced over display rules) | `internal/server`, `internal/webuilane` | `TestWebUIReviewRatchet`, `TestWebUIComposerBudget` | one style for the client: no inline style attribute, no nested ternary, no timer literal can land without raising a reviewed ceiling |
