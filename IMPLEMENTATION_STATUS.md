...
| Item | Current evidence | Completion boundary |
| --- | --- | --- |
| Indexed cache-target plan | Compiled graphs own output ordinals, alias contracts, indexed target slots, and indexed retained values; model plans compile immutable layer-cache schema templates; decode only materializes token dimensions; capacity shapes remain request-local | Compiled session graph also owns capacity shapes; decode fills target slices without page-capacity recomputation |
