...
| Video generation | Typed oscillator, Wan, and LiveEdit recipes; GIF artifact publication; CUDA-resident Wan denoising and VAE encoding/decoding; retained LiveEdit text projection and bounded attention history | Un-0 publishes six real-artifact frames as a 64x64 GIF byte-identical to adaptive_new. Wan has verified full-clip execution. LiveEdit executes all 30 blocks through a retained recipe session and publishes a deterministic five-frame GIF in the bounded real-data gate. Full-size native comparison remains open. |
...
Full-size LiveEdit comparison, exact full-sequence Unlimited OCR comparison, comparable
