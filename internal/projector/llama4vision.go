...
func (r *Llama4VisionRunner) encodeTile(ctx context.Context, input Llama4VisionTile) (reference.Value, error) {
	...
	hidden := hostmath.LinearF64New(input.PixelValues, patchWeight.Data, patchBias, patchRows, patchWidth, r.spec.Hidden)
	...
	adapted := hostmath.LinearF64New(shuffled, mlp1.Data, nil, mergePlan.outputRows, shuffleWidth, r.spec.AdapterIntermediate)
	hostmath.GELUTanhInPlace(adapted)
	mlp2, err := r.load(ctx, "mm.model.mlp.2.weight")
	if err != nil {
		return reference.Value{}, err
	}
	adapted = hostmath.LinearF64New(adapted, mlp2.Data, nil, mergePlan.outputRows, r.spec.AdapterIntermediate, r.spec.AdapterHidden)
	hostmath.GELUTanhInPlace(adapted)
	projection, err := r.load(ctx, "mm.model.fc.weight")
	if err != nil {
		return reference.Value{}, err
	}
	output := hostmath.LinearF64New(adapted, projection.Data, nil, mergePlan.outputRows, r.spec.AdapterHidden, r.spec.OutputHidden)
...
func (r *Llama4VisionRunner) runLayer(ctx context.Context, hidden []float32, gridH, gridW, layer int) error {
	...
	projected := hostmath.LinearF64New(attention, outWeight.Data, outBias, rows, r.spec.Hidden, r.spec.Hidden)
	for index := range hidden {
	...
	up := hostmath.LinearF64New(norm, upWeight.Data, upBias, rows, r.spec.Hidden, r.spec.Intermediate)
	...
	down := hostmath.LinearF64New(up, downWeight.Data, downBias, rows, r.spec.Intermediate, r.spec.Hidden)
	for index := range hidden {
...
func (r *Llama4VisionRunner) projectQKV(ctx context.Context, input []float32, rows int, prefix string, layer int) ([]float32, error) {
	if r.spec.FusedQKV[layer] {
		...
		return hostmath.LinearF64New(input, weight.Data, bias, rows, r.spec.Hidden, 3*r.spec.Hidden), nil
	}
	...
	for partIndex, part := range []string{"q", "k", "v"} {
		...
		projected := hostmath.LinearF64New(input, weight.Data, bias, rows, r.spec.Hidden, r.spec.Hidden)
