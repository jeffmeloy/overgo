...
| Needle seq2seq | Partial adaptive forward/backward | Fingerprinted real weights/JAX oracle; retained load+generate envelope <=250 vs 420 ms. The typed text recipe uses the shared SentencePiece tokenizer and emits grounded JSON for the model-card weather tool-call example. Candidate execution published a recipe-bound gate/run pair; activation and active replay passed at the experimental tier. A shared Windows child-process harness now measures three matched cold one-token runs. Direct safetensors-to-F32 loading removed a tensor-sized temporary and reduced Overgo's best observed peak from 147.926 to 138.055 MiB; adaptive_new measured 120.887 MiB | Exact numeric parity; real text generation; active recipe; wall lead; process-memory loss measured | Reduce the matched process peak below 120.887 MiB without changing output; approved fine-tuning objective and dataset |
...
| LiveEdit video edit | Adaptive complete experimental route; best 109.5 s, 20.61 GB; Python 60.1 s | The shared source-codec boundary now derives causal `[1,stride,...]` frame chunks, latent geometry, channel-major frame spans, and recipe-owned latent normalization. Encoder graph execution, denoise integration, and decode remain incomplete | Source geometry and normalization ported; execution gap | Compile and execute the causal VAE encoder from checkpoint bindings; retain source latent through denoise; beat Python without temporal degradation |
