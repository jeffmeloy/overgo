...
func TestStaticFailurePreventsAcceptance(t *testing.T) {
	for _, failed := range []string{"modern-go", "vet", "build", ""} {
		for _, batched := range []bool{false, true} {
			...
			t.Run(name, func(t *testing.T) {
				...
				for _, check := range checks {
					...
					if name != "acceptance" && name != "test" && name != "test-owners" && name != "test-device" && name != "device" && name != automationcheck.WebUICheckName && name != "commit" && !slices.ContainsFunc(batch.Checkpoints, func(c plan.VerificationCheckpoint) bool { return c.GateCheckName() == name }) {
						continue
					}
