...
func TestDomainRoutedSuites(t *testing.T) {
	...
	if _, declared, err := EvalDomains(ctx, store, model); err != nil || declared {
		t.Fatalf("undeclared model = (%t, %v)", declared, err)
	}
	if _, err := DeclareEvalDomains(ctx, store, model, []string{"dna"}); err != nil {
		t.Fatal(err)
	}
	domains, declared, err := EvalDomains(ctx, store, model)
	if err != nil || !declared || len(domains) != 1 || domains[0] != "dna" {
		t.Fatalf("declared domains = (%v, %t, %v)", domains, declared, err)
	}
	if _, err := DeclareEvalDomains(ctx, store, model, []string{"dna", "text"}); err != nil {
		t.Fatal(err)
	}
	if domains, _, _ := EvalDomains(ctx, store, model); len(domains) != 2 {
