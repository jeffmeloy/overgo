...
func renderMarkdown(manifest apimanifest.Manifest) string {
	...
	fmt.Fprintf(&out, "Source identity `%s`; %d binaries, %d document contracts, %d protocols, %d authorities.\n\n",
		manifest.SourceIdentity, len(manifest.Binaries), len(manifest.Documents), len(manifest.Protocols), len(manifest.Authorities))
