...
func NewUniPCSampler(sigmas []float32, elements int) (*UniPCSampler, error) {
	...
	if len(sigmas) < 2 {
		return nil, fmt.Errorf("unipc sampler: need at least 2 sigmas, got %d", len(sigmas))
	}
	for i, sigma := range sigmas {
		if sigma < 0 || sigma >= 1 || (i < len(sigmas)-1 && sigma <= 0) {
			return nil, fmt.Errorf("unipc sampler: bad sigma[%d]=%g", i, sigma)
		}
	}
	return &UniPCSampler{
...
// NoisePlan: launch geometry of the counter-based normal source (the
// capture engine's torch.cuda randn kernel).
...
// FillNormalNoise: counter-based standard normals. Thread idx owns Philox
// subsequence idx; each generation yields four normals distributed with a
// blockDim*gridDim stride, matching the reference kernel's unrolled layout.
func FillNormalNoise(dst []float32, plan NoisePlan) error {
	if plan.Grid <= 0 || plan.Block <= 0 || plan.Unroll != 4 {
		return fmt.Errorf("noise plan grid=%d block=%d unroll=%d is invalid", plan.Grid, plan.Block, plan.Unroll)
	}
	numel := int64(len(dst))
...
func philoxInit(seed, subsequence, offset uint64) philoxCounter {
	...
	// Skip-ahead in whole 4-tuples (offsets are 4-aligned by construction).
	advance := offset / 4
...
func philoxIncrement(c philoxCounter) philoxCounter {
	c.x++
	if c.x == 0 {
		c.y++
		if c.y == 0 {
...
// philox4x32x10: the Philox 4x32-10 block cipher keyed by the seed.
func philox4x32x10(c philoxCounter, seed uint64) (uint32, uint32, uint32, uint32) {
	const (
	...
	for round := 0; round < 10; round++ {
		productA := uint64(multiplierA) * uint64(c.x)
		productB := uint64(multiplierB) * uint64(c.z)
...
// Box-Muller uniform grids (the capture engine's curand literals).
const (
	uniformScale  = float32(2.3283064e-10) // 2^-32
	uniformOffset = uniformScale / 2
	angularScale  = float32(2.3283064e-10 * 6.2831855) // 2^-32 * 2*pi
	angularOffset = angularScale / 2
)

// boxMullerNormalPair: (s*sin(v), s*cos(v)) with s = sqrt(-2 ln u), each
// operation in float32 like the device kernel.
func boxMullerNormalPair(x, y uint32) (float32, float32) {
	...
	s := float32(math.Sqrt(float64(float32(-2) * float32(math.Log(float64(u))))))
	sin := float32(math.Sin(float64(v)))
	cos := float32(math.Cos(float64(v)))
	return s * sin, s * cos
}
