...
func TestParameterizedDecodeCapacityStaysWithinPageClass(t *testing.T) {
	...
	runner := &Runner{preparedModel: preparedModel{
		...
		plan: model.ModelPlan{Profile: model.ArchitectureProfile{
	...
	if _, ok := runner.parameterizedDecodeCapacity(appendFor(2), deviceOutputPlan{}); ok {
		t.Fatal("buffered decode admitted parameterized session")
	}
}
...
func TestParameterizedDecodeAttributesCoverDynamicNodes(t *testing.T) {
	...
	session := &deviceDecodeSession{
		compiled: compiled, nodes: builder.Nodes(),
	}
	if _, err = parameterizedDecodeAttributes(session, []uint32{3}, 3); err != nil {
		t.Fatal(err)
	}
}
