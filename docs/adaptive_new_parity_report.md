...
| Unlimited OCR | Native BF16 image/text golden: 277 prompt IDs, 811 generated IDs, 29 OCR rows; `crop_mode=false` | Shared DeepSeek OCR CUDA projector plus shared DeepSeek2/MoE inference: exact prompt IDs, 273 finite projected tokens, exact 128-token output prefix, and validated 29-row oracle. Fresh focused run: 10.039 s projection and 39.144 s prefix generation | Real production prefix parity | Add the native 35-gram/128-window repetition policy to shared device sampling; then promote the complete 811-token text/layout replay and matched peak |
...
| SenseNova image/edit | Adaptive exact Go replay: 206.010 s, 19.50 GB; Python 366.6 s, 47.58 GiB | Fingerprinted retained-session two-step core: boundary cosine >=0.99914, velocity >=0.99863, final state >=0.99832; fresh cold body 8.117 s, warm body 4.048 s, 0.710 GiB peak versus adaptive 6.88 s; cold <=8.6 s and warm <=6.2 s are gated separately | Warm-body lead; cold-body regression bounded; production gap | Remove retained-prefix copies; bind production decode/publication; edit route still needs its absent source PNG |
...
| Unlimited OCR image/text | Native BF16 811-token golden | 10.039 s projection + 39.144 s exact 128-token prefix; 273 image tokens | Functional prefix parity; full no-repeat replay and matched peak open |
