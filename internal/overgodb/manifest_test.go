...
func TestManifestRecommitReplaysAsFixedPoint(t *testing.T) {
	...
	defer store.Close()
	got, ok, err := store.Manifest(t.Context(), manifest.ID)
...
func TestManifestAndLocationReplay(t *testing.T) {
	...
	parents, err := store.Parents(t.Context(), manifest.ID)
	if err != nil || len(parents) != 1 || parents[0].Parent != component.ID {
		t.Fatalf("manifest parents = (%+v, %v)", parents, err)
	}
	locations, err := store.Locations(t.Context(), component.ID)
...
func TestLocationRemoval(t *testing.T) {
	...
	locations, err := store.Locations(t.Context(), descriptor.ID)
