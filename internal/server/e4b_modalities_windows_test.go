...
func TestE4BHTTPModalities(t *testing.T) {
	...
	for caseIndex, test := range cases {
		golden := oracle.Cases[caseIndex]
		if golden.Name != test.name || len(golden.Outputs) == 0 || len(golden.PromptIDs) == 0 {
		...
		for _, route := range []string{"/completion", "/v1/chat/completions", "/v1/responses"} {
			t.Run(test.name+route, func(t *testing.T) {
				...
				if !slices.Equal(trace.ids, golden.PromptIDs) {
					t.Errorf("prepared prompt differs from native reference: got %d tokens, want %d", len(trace.ids), len(golden.PromptIDs))
				}
				if !slices.Contains(golden.Outputs, actual) {
					t.Errorf("complete output differs from native reference: got %q, want one of %q", actual, golden.Outputs)
				}
			})
		}
	}
