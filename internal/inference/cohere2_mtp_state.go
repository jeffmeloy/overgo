...
func (r *Runner) SaveCohere2MTPSession(session *Cohere2MTPSession) ([]byte, error) {
	...
	if session.targetModel == [32]byte{} {
		return nil, errors.New("inference: Cohere2-MoE MTP target binding is missing")
	}
	...
	if draftTokens > 0 {
		...
		if err != nil {
			return nil, fmt.Errorf("inference: save Cohere2-MoE MTP layer cache: %w", err)
		}
	}
	...
	if total > uint64(maxIntValue()) {
		return nil, errors.New("inference: Cohere2-MoE MTP state exceeds addressable memory")
	}
	output := make([]byte, int(total))
	copy(output, cohere2MTPStateMagic)
...
func (r *Runner) LoadCohere2MTPSession(data []byte) (*Cohere2MTPSession, error) {
	...
	if len(data) < qwen35MTPStateHeader {
		return nil, errors.New("inference: Cohere2-MoE MTP state is truncated")
	}
	if string(data[:8]) != cohere2MTPStateMagic {
		return nil, errors.New("inference: Cohere2-MoE MTP state has invalid magic or version")
	}
	draftModel, err := r.sessionModelSignature()
	if err != nil {
		return nil, err
	...
	if string(data[8:40]) != string(draftModel[:]) {
		return nil, errors.New("inference: Cohere2-MoE MTP state belongs to a different draft model")
	}
	var targetModel [32]byte
	copy(targetModel[:], data[40:72])
	if targetModel == [32]byte{} {
		return nil, errors.New("inference: Cohere2-MoE MTP state target binding is invalid")
	}
	...
	if position < mtpStart || position == math.MaxUint32 ||
		...
		return nil, errors.New("inference: Cohere2-MoE MTP state positions are invalid")
	}
	hiddenBytes := uint64(r.spec.EmbeddingLength) * 4
	...
	if hiddenBytes > payload || trunkLength > payload-hiddenBytes ||
		...
		return nil, errors.New("inference: Cohere2-MoE MTP state payload lengths are invalid")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("inference: load Cohere2-MoE MTP trunk cache: %w", err)
	}
	...
	if mtpLength > 0 {
		...
		if cacheErr != nil {
			return nil, fmt.Errorf("inference: load Cohere2-MoE MTP layer cache: %w", cacheErr)
		}
		if len(mtpCache.Layers) != 1 || mtpCache.Tokens != position-mtpStart ||
			mtpCache.Position != position {
			return nil, errors.New("inference: Cohere2-MoE MTP layer cache metadata is invalid")
		}
